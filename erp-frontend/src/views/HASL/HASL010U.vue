                    ...
                    <tbody>
                        ...
                        <template v-else-if="selectedRow.typeacct === '090'">
                            ...
                            <template v-if="String(selectedRow.typedc || '').trim().toUpperCase() === String(selectedRow.dbcr || '').trim().toUpperCase()">
                                ...
                                <tr v-if="String(selectedRow.typedc || '').trim().toUpperCase() === 'D' && String(selectedRow.dbcr || '').trim().toUpperCase() === 'D'">
                                    <th class="bg-light border-bottom text-end pe-3 text-primary fw-bold">카&nbsp;&nbsp;&nbsp;&nbsp;드</th>
                                    <td class="border-bottom">
                                        <div class="input-group input-group-sm">
...
async function searchSlips() {
  try {
    const res = await api.post('/api/hasl/HASL_010U_STR', {
      ...
      fromdt: searchParams.fromdt.replace(/-/g, ''), todt: searchParams.todt.replace(/-/g, ''),
...
onMounted(async () => {
  ...
  nextTick(async () => {
    ...
    if (route.query.slipymd && route.query.slipno) {
        const ymd = String(route.query.slipymd);
        ...
        try {
            const res = await api.post('/api/hasl/HASL_010U_STR', {
              actkind: 'S', cmpycd: authStore.cmpycd,
              slipymd: ymd, toymd: ymd,
              business: ''
            });
            const list = res.data || [];
            grid1?.setData(list);

            const target = list.find((item: any) => item.slipno === route.query.slipno);
            if (target) {
                fetchDetail(target);
            } else {
                vAlert('해당 전표를 찾을 수 없습니다.');
            }
        } catch (e) { vAlertError('전표 조회 실패'); }
    } else {
        searchSlips();
    }
  });
