<!--
	...
	작성일자	: 2025.02.24
	작성자	    : AI Assistant
	설명        : 특정 계정과목에 대해 관리번호별 이월액, 증가액, 감소액 및 기말 잔액을 조회
	=============================================================
-->
	...
	<div class="erp-container">
		...
		<div class="erp-header d-flex justify-content-between align-items-center border-bottom bg-white py-2 px-3 sticky-top shadow-sm flex-shrink-0">
			<div class="fw-bold text-dark d-flex align-items-center" style="font-size: 14px;">
				...
				<span class="text-primary fw-bolder">증감잔액명세서-관리번호 (HASL630S)</span>
			</div>
...
const search = async () => {
	...
	try {
		...
		const data = (res.data || []).map((row: any) => ({
			...row,
			mgtno: row.col0 || row.mgtno,
			mgtnm: row.col1 || row.mgtnm,
	...
	if (mainGridRef.value) {
		mainGrid = new Tabulator(mainGridRef.value, {
			...
			columns: [
				...
				{
					title: "관 리 번 호 명", field: "mgtnm", widthGrow: 2,
	...
	if (searchForm.acctcd) {
		try {
			...
			if (res.data?.length > 0) {
				...
				searchForm.mgtgbn = res.data[0].TYPEMGT
				search()
			}
		} catch (e) {}
	}
