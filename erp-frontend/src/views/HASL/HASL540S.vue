<!--
	=============================================================
	?ÑÎ°úÍ∑∏Îû®Î™?: Î≥¥Ï°∞?êÏû• (HASL540S)
	?ëÏÑ±?ºÏûê	: 2025.02.24
	?ëÏÑ±??    : AI Assistant
	?§Î™Ö        : ?πÏ†ï Í≥ÑÏ†ïÍ≥ºÎ™©???Ä???ÅÏÑ∏ ?ÑÌëú ?¥Ïó≠ Î∞??îÏï° Ï°∞Ìöå (HSOD100U ?úÏ? ?îÏûê??Î∞??åÎ¨∏???êÏπô ?ÅÏö©)
	=============================================================
-->
  ...
  <div class="erp-container d-flex flex-column h-100 bg-white">
    ...
    <div class="erp-header d-flex justify-content-between align-items-center flex-shrink-0 border-bottom bg-white py-2 px-3 sticky-top shadow-sm">
      <div class="fw-bold ps-1 text-dark d-flex align-items-center" style="font-size: 14px;">
        ...
        <span class="text-primary fw-bolder">Î≥¥Ï°∞?êÏû• (HASL540S)</span>
      </div>
    ...
    <div class="flex-grow-1 overflow-hidden p-2 d-flex flex-column gap-2 bg-light main-content-wrapper">
      ...
      <div class="card border shadow-sm flex-shrink-0 overflow-hidden">
        <div class="card-body p-0 bg-white">
          <table class="erp-table-dense" width="100%">
            ...
            <tbody>
              <tr>
                <th class="text-center bg-light border-end">Í≥ÑÏ†ïÍ≥ºÎ™©</th>
                <td class="border-end px-2">
                  <div class="input-group input-group-sm" style="max-width: 320px;">
                    ...
                    <input v-model="search_form.acctnm" type="text" class="form-control" placeholder="Í≥ÑÏ†ïÎ™??ÖÎ†•" @keydown.enter="open_help('ACCT')" />
      ...
      <div class="card border shadow-sm flex-grow-1 d-flex flex-column overflow-hidden grid-container-right">
        <div class="card-header bg-white py-1 px-3 border-bottom text-secondary">
          <span class="fw-bold small"><i class="bi bi-grid-3x3-gap-fill me-2 text-primary"></i> Í≥ÑÏ†ïÎ≥??ÅÏÑ∏ Í±∞Îûò ?¥Ïó≠</span>
        </div>
...
// ?í° ?†Ïßú ?¨Îß∑???®Ïàò (20260601 -> 2026-06-01)
...
const search = async () => {
	if (!search_form.acctcd) {
		v_alert_error('Í≥ÑÏ†ïÍ≥ºÎ™©???†ÌÉù??Ï£ºÏã≠?úÏò§.')
		return
	}
	...
	try {
		const res = await api.post('/api/hasl/HASL_540S_STR', {
			...
			fromdt: search_form.fromdt.replace(/-/g, ''), // ?í° XML Í∑úÍ≤©??ÎßûÏ∂§
			todt: search_form.todt.replace(/-/g, '')  // ?í° XML Í∑úÍ≤©??ÎßûÏ∂§
		})
		...
		if (raw_data.length > 0) {
			...
			while (i < raw_data.length) {
				...
				processed_data.push({ acctymd: '', descnm: '??  Í≥?, dbamt: db_mm_amt, cramt: cr_mm_amt, janamt: null, is_monthly: true })
				db_mm_tot += db_mm_amt; cr_mm_tot += cr_mm_amt;
				processed_data.push({ acctymd: '', descnm: '??  Í≥?, dbamt: db_mm_tot, cramt: cr_mm_tot, janamt: jan_amt, is_total: true })
			}
		}
		main_grid?.setData(processed_data)
		v_alert(processed_data.length > 0 ? 'Ï°∞Ìöå?òÏóà?µÎãà??' : '?∞Ïù¥?∞Í? Ï°¥Ïû¨?òÏ? ?äÏäµ?àÎã§.')
	} catch (e) { v_alert_error('Ï°∞Ìöå Ï§??§Î•ò Î∞úÏÉù') }
}
...
const go_slip_detail = (slip_no: string) => {
	...
	add_dynamic_route(pgmid, '?ÑÌëúÍ¥ÄÎ¶?, 'HASL')
...
const print = () => {
	if (!search_form.acctcd) return v_alert_error('Í≥ÑÏ†ïÍ≥ºÎ™©??Î®ºÏ? ?†ÌÉù?òÏÑ∏??')
...
_on_mounted(async () => {
	if (main_grid_ref.value) {
		main_grid = new tabulator(main_grid_ref.value, {
			...
			columns: [
				{ title: "?ºÏûê", field: "acctymd", width: 90, hozAlign: "center", formatter: (cell) => { const v = cell.getValue(); return v && v.length === 8 ? `${v.substring(2, 4)}.${v.substring(4, 6)}.${v.substring(6, 8)}` : '' } },
				{ title: "?ÅÏöî / ?∏Î??¥Ïó≠", field: "descnm", widthGrow: 2.5, formatter: (cell) => { const d = cell.getData(); return d.is_data ? `<div>${cell.getValue()}</div><div class="small text-secondary fw-normal mt-1" style="font-size: 11px;">${d.detail_str}</div>` : cell.getValue() } },
				{ title: "?ÑÌëúÎ≤àÌò∏", field: "slipno", width: 120, hozAlign: "center", formatter: (cell) => cell.getValue() ? `<span class="text-primary text-decoration-underline cursor-pointer">${cell.getValue()}</span>` : '', cellClick: (e, cell) => { go_slip_detail(cell.getValue()) } },
				{ title: "Ï∞®Î?", field: "dbamt", width: 120, hozAlign: "right", formatter: "money" },
				{ title: "?ÄÎ≥Ä", field: "cramt", width: 120, hozAlign: "right", formatter: "money" },
				{ title: "?îÏï°", field: "janamt", width: 120, hozAlign: "right", formatter: "money" }
			],
