<!--
	...
	작성일자	: 2025.02.24
	작성자	    : AI Assistant
	설명        : 특정 계정과목에 대한 상세 전표 내역 및 잔액 조회 (세부 관리항목 포함)
	=============================================================
-->
			...
			while (i < rawData.length) {
				...
				while (i < rawData.length && String(rawData[i].col0).substring(0, 6) === currentYm) {
					...
					for (let idx = 12; idx <= 17; idx++) {
						...
						if (val && String(val).trim().length > 0) details.push(String(val).trim())
					}
					...
					processedData.push({
						acctymD: row.col0,
						descnm: row.col1 + (row.col11 && row.col11 !== '0' ? ` (${row.col11})` : ''),
						SLIPNO: row.col2,
						dbamt: db,
						cramt: cr,
						janamt: janAmt,
...
const print = () => {
	...
	window.open(`/api/hasl/HASL_540P?${params}`, 'SubsidiaryLedgerPrint', 'width=800,height=800,scrollbars=yes')
}
