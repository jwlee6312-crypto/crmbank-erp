<!--
	=============================================================
	?ÑÎ°úÍ∑∏Îû®Î™?: Ï¥ùÍ≥Ñ?ïÏõê??(HASL530S)
	?ëÏÑ±?ºÏûê	: 2025.02.24
	?§Î™Ö        : Í≥ÑÏ†ïÍ≥ºÎ™©Î≥??ÅÏÑ∏ Í±∞Îûò ?¥Ïó≠ Î∞??îÍ≥Ñ, ?ÑÍ≥Ñ Ï°∞Ìöå (HSOD100U ?úÏ? ?îÏûê??Î∞??åÎ¨∏???êÏπô ?ÅÏö©)
	=============================================================
-->
  ...
  <div class="erp-container d-flex flex-column h-100 bg-white">
    ...
    <div class="erp-header d-flex justify-content-between align-items-center flex-shrink-0 border-bottom bg-white py-2 px-3 sticky-top shadow-sm">
      <div class="fw-bold ps-1 text-dark d-flex align-items-center" style="font-size: 14px;">
        ...
        <span class="text-primary fw-bolder">Ï¥ùÍ≥Ñ?ïÏõê??(HASL530S)</span>
      </div>
    ...
    <div class="flex-grow-1 overflow-hidden p-2 d-flex flex-column gap-2 bg-light main-content-wrapper">
      ...
      <div class="card border shadow-sm flex-grow-1 d-flex flex-column overflow-hidden grid-container-right">
        <div class="card-header bg-white py-1 px-3 border-bottom text-secondary">
          <span class="fw-bold small"><i class="bi bi-grid-3x3-gap-fill me-2 text-primary"></i> Í≥ÑÏ†ïÎ≥?Í±∞Îûò ?¥Ïó≠</span>
        </div>
...
const search = async () => {
	...
	try {
		...
		if (processed_data.length > 0) v_alert('Ï°∞Ìöå?òÏóà?µÎãà??')
		else v_alert('?∞Ïù¥?∞Í? Ï°¥Ïû¨?òÏ? ?äÏäµ?àÎã§.')
...
// ?? ?ÅÏÑ∏ ?¥Îèô (Í≤ΩÎ°ú ?òÏ†ï Î∞??ôÏ†Å ?ºÏö∞???±Î°ù)
const go_detail = (ymd: string) => {
	if (!ymd) return
    const pgmid = 'HASL540S'
    add_dynamic_route(pgmid, 'Î≥¥Ï°∞?êÏû•', 'HASL')
	...
	if (main_grid_ref.value) {
		main_grid = new tabulator(main_grid_ref.value, {
			...
			columns: [
				...
				{ title: "Ï∞®Î?", field: "dbamt", width: 130, hozAlign: "right", formatter: "money", formatterParams: { precision: 0 } },
				{ title: "?ÄÎ≥Ä", field: "cramt", width: 130, hozAlign: "right", formatter: "money", formatterParams: { precision: 0 } },
				{ title: "?îÏï°", field: "janamt", width: 130, hozAlign: "right", formatter: "money", formatterParams: { precision: 0 } }
			],
			rowFormatter: (row) => {
				const d = row.getData()
