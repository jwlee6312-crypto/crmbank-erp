<!--
	...
	작성일자	: 2025.02.24
	작성자	    : AI Assistant
	설명        : 지정된 년/월의 차변/대변 합계 및 현금 시재 현황 조회
	=============================================================
-->
			...
			<div class="card border shadow-sm flex-shrink-0 bg-white overflow-hidden">
				<table class="erp-table-full small text-center">
					...
					<thead>
						...
						<tr class="bg-light border-bottom">
							<th class="border-end py-1 fw-normal">전월잔액</th>
							<th class="border-end py-1 fw-normal">당월증가</th>
							<th class="border-end py-1 fw-normal">당월감소</th>
							<th class="border-end py-1 fw-normal text-primary">당월잔액</th>
						</tr>
					</thead>
					<tbody>
						<tr>
...
const print = () => {
	...
	window.open(`/api/hasl/HASL_520P?${params}`, 'MonthlyStatementPrint', 'width=800,height=800,scrollbars=yes')
}
...
const goAccountDetail = (acctCd: string) => {
	...
	const lastDayDate = new Date(Number(searchForm.yy), Number(searchForm.mm), 0)
	const lastDay = `${searchForm.yy}-${searchForm.mm}-${String(lastDayDate.getDate()).padStart(2, '0')}`

	router.push({
		path: '/HASL/HASL540S',
		query: { frymd: firstDay, toymd: lastDay, acctcd: acctCd }
	})
}
