...
const search = async () => {
  ...
  try {
    ...
    const processedData = rawData.map((row: any, index: number) => {
      const code = row.col0
      const name = row.col1
      const rstyn = row.col8
      const salestot_b = Number(row.COL9 || 0)
      const salestot_c = Number(row.col10 || 0)
      const salestot_t = Number(row.col11 || 0)

      const bAmt = Number(row.col2 || 0) !== 0 ? Number(row.col2) : Number(row.col3 || 0)
      const cAmt = Number(row.col4 || 0) !== 0 ? Number(row.col4) : Number(row.col5 || 0)
      const tAmt = Number(row.col6 || 0) !== 0 ? Number(row.col6) : Number(row.col7 || 0)
