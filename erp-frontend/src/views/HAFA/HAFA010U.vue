<!--
	...
	작성일자	: 2025.02.24
	작성자	    : AI Assistant
	설명        : 고정자산 취득, 변동, 처분 등 이력 관리 (상하단 폰트 12px 통일 및 표준 디자인 적용)
	=============================================================
-->
...
<template>
  ...
  <div class="erp-container">
    ...
    <!-- 💡 2. 메인 컨텐츠 영역 (gap-2 적용) -->
    <div class="flex-grow-1 overflow-auto p-2 d-flex flex-column gap-2">

      <!-- 🔍 2-1. 조회 조건 카드 (표준 erp-table-full) -->
      <div class="card border-0 shadow-sm overflow-hidden flex-shrink-0">
        <div class="card-header bg-white py-1 px-3 border-bottom d-flex align-items-center">
          <div class="fw-bold small text-dark"><i class="bi bi-search me-2 text-primary"></i>조회 조건</div>
        </div>
              ...
              <tr>
                ...
                <td>
                  <div class="input-group">
                    <input v-model="searchForm.acctcd" type="text" class="form-control text-center bg-light fw-bold" style="max-width: 60px;" readonly />
                ...
                <td>
                  <div class="d-flex align-items-center gap-1">
                    <input v-model="searchForm.frymd" type="date" class="form-control" style="width: 135px;" />
                    <span class="text-muted small fw-bold">~</span>
                    <input v-model="searchForm.toymd" type="date" class="form-control" style="width: 135px;" />
                  </div>
                </td>
      ...
      <!-- 📝 2-2. 상세 정보 입력 카드 (HSOD100U 고밀도 스타일) -->
      <div class="card border-0 shadow-sm overflow-hidden flex-shrink-0">
        ...
        <div class="card-body p-0 bg-white">
          <table class="erp-table-full border-0">
            ...
            <tbody>
              <tr>
                <th class="required">처리구분</th>
                <td>
                  <select v-model="formData.proctype" class="form-select">
                    <option v-for="item in proctypeOptions" :key="item.code" :value="item.code">{{ item.name }}</option>
                  </select>
                </td>
                <th>자산코드</th>
                <td>
                  <div class="input-group">
                    ...
                    <button class="btn" @click="openHelp('ASET')" :disabled="formData.proctype === '010'"><i class="bi bi-search"></i></button>
                  </div>
                </td>
                <th class="required">자산명</th>
                <td><input v-model="formData.asetnm" type="text" class="form-control" placeholder="자산 명칭" /></td>
              </tr>
              <tr>
                <th class="required">계정과목</th>
                <td>
                  <div class="input-group">
                    <input v-model="formData.acctcd" type="text" class="form-control text-center bg-light fw-bold" style="max-width: 60px;" readonly />
                ...
                <th class="required">발생일</th>
                <td><input v-model="formData.procymd" type="date" class="form-control" /></td>
                <th class="required">수량/금액</th>
                <td>
                  <div class="d-flex gap-1 align-items-center">
                    <input v-model="formData.procqty" type="number" class="form-control text-end" style="width: 70px;" />
                    <input v-model="formData.procamt" type="number" class="form-control text-end fw-bold text-primary" placeholder="0" />
                  </div>
                </td>
              </tr>
              <tr>
...
const fetchOptions = async () => {
	try {
		const [resP, resD, resC] = await Promise.all([
			api.post('/api/ha00/HA00_00P_STR', { gubun: 'E0', search: '260' }),
			api.post('/api/ha00/HA00_00P_STR', { gubun: 'E0', search: '250' }),
			api.post('/api/ha00/HA00_00P_STR', { gubun: 'E0', search: '110' })
		])
		proctypeOptions.value = resP.data; dprstypeOptions.value = resD.data; costtypeOptions.value = resC.data
...
async function search() {
	...
	try {
		...
		const data = (res.data || []).map((r: any) => ({ ...r, procymd: r.procymd ? `${r.procymd.slice(0,4)}-${r.procymd.slice(4,6)}-${r.procymd.slice(6,8)}` : '' }))
...
async function deleteData() {
	...
	try {
		const res = await api.post('/api/hafa/HAFA_010U_STR', { ...formData, actkind: 'D', cmpycd: authStore.cmpycd, userid: authStore.userid })
...
function openHelp(type: string, target?: string) {
	if (type === 'ACCT') {
		...
		Object.assign(modalProps, { title: '자산 선택', path: '/api/hafa/HELP_asetcd_LTD_STR', data: { acctcd: formData.acctcd, search: '' }, columns: [{ title: '코드', field: 'asetcd', width: 100 }, { title: '자산명', field: 'asetnm', width: 200 }],
			onConfirm: (d: any) => { Object.assign(formData, { asetcd: d.asetcd, asetnm: d.asetnm, legalyy: d.legalyy, deptcd: d.deptcd, deptnm: d.deptnm, costtype: d.costtype, dprstype: d.dprstype, asetrate: d.asetrate }) }
		})
	} else if (type === 'DEPT') {
...
onMounted(() => {
	...
	if (mainGridRef.value) {
		mainGrid = new Tabulator(mainGridRef.value, {
			...
			columns: [
				{ title: "발생일", field: "procymd", width: 100, hozAlign: "center" },
				{ title: "구분", field: "procnm", width: 80, hozAlign: "center", cssClass: "text-primary fw-bold" },
				...
				{ title: "상태", field: "useyn", width: 60, hozAlign: "center", formatter: (c) => c.getValue() === 'Y' ? '정상' : '<span class="text-danger">삭제</span>' }
			]
		})
