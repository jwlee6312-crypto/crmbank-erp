...
<template>
	...
	<div class="erp-container">
		...
		<!-- 🔍 검색/입력 조건 영역 (표준 erp-label 적용) -->
		<div class="p-2 pb-0 flex-shrink-0">
			<div class="card border shadow-sm bg-white overflow-hidden">
				<div class="card-body p-2 bg-light">
					<div class="d-flex align-items-center flex-wrap gap-3">
						<div class="d-flex align-items-center">
							<span class="erp-label">예산연도</span>
							<select v-model="searchForm.bugtyy" class="form-select" style="width: 100px;" @change="search">
						...
						<div class="d-flex align-items-center">
							<span class="erp-label">예산코드</span>
							<div class="input-group" style="width: 220px;">
...
const search = async () => {
	...
	try {
		...
		const data = (res.data || []).map((row: any) => ({
		    mm: row.col0,
			gubun: row.col5,
			last_amt: Number(row.col1 || 0),
			req_amt: Number(row.col2 || 0),
			adj_amt: Number(row.col3 || 0),
			alloc_amt: Number(row.col4 || 0)
		}))
...
function openHelp(type: 'DEPT' | 'BUGT') {
	if (type === 'DEPT') {
		...
		Object.assign(modalProps, {
			...
			data: { gubun: 'B0', cmpycd: authStore.cmpycd, search: searchForm.bugtnm },
	...
	if (mainGridRef.value) {
		mainGrid = new Tabulator(mainGridRef.value, {
			...
			columns: [
				...
				{ title: "전년사용액", field: "last_amt", width: 150, hozAlign: "right", formatter: "money", formatterParams: { precision: 0 } },
				{
					title: "신청액", field: "req_amt", width: 180, hozAlign: "right",
				...
				{ title: "배정액", field: "alloc_amt", width: 150, hozAlign: "right", formatter: "money", formatterParams: { precision: 0 } },
