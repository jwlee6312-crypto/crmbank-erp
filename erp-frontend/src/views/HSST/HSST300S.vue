...
<template>
	...
	<div class="hsst300s-wrapper d-flex flex-column h-100 bg-white p-0">
		...
		<div class="erp-footer bg-dark text-white py-2 px-4 shadow-lg sticky-bottom flex-shrink-0">
			<div class="row align-items-center w-100">
				<div class="col-md-2 small">조회 건수: <span class="fw-bold text-info">{{ rowCount }}</span> 건</div>
				<div class="col-md-10 text-end">
					<span class="me-4 small opacity-75">당월 매출합계: <span class="fw-bold text-white ms-1">{{ formatNumber(totals.SMTOT) }}</span></span>
					<span class="fs-5 ms-2 fw-light">누계 총액: <span class="fw-bold text-warning ms-2">{{ formatNumber(totals.SYTOT) }}</span> 원</span>
				</div>
			</div>
		</div>
	</div>
...
const handleExcel = () => mainGrid?.download("xlsx", "거래처분류별판매현황.xlsx")
