...
<template>
	...
	<div class="hsst150s-wrapper d-flex flex-column h-100 bg-white p-0">
		...
		<div class="erp-footer bg-dark text-white py-2 px-4 shadow-lg sticky-bottom flex-shrink-0">
			<div class="row align-items-center w-100">
				<div class="col-md-2 small">조회 항목: <span class="fw-bold text-info">{{ rowCount }}</span> 건</div>
				<div class="col-md-10 text-end">
					<span class="me-4 small opacity-75">당월 매출합계: <span class="fw-bold text-white ms-1">{{ formatNumber(totals.SMAMT) }}</span></span>
					<span class="fs-5 ms-2 fw-light">누계 매출액: <span class="fw-bold text-warning ms-2">{{ formatNumber(totals.SYAMT) }}</span> 원</span>
				</div>
			</div>
		</div>
	</div>
...
const fetchData = async () => {
	try {
		const res = await api.post('/api/hsst/HSST_150S_STR', {
			...searchForm,
...
const handleExcel = () => mainGrid?.download("xlsx", "품목분류별판매현황.xlsx")
