...
<template>
	...
	<div class="hsst610s-wrapper d-flex flex-column h-100 bg-white p-0">
		<!-- 🚀 1, 11, 12. 상단 액션 바 -->
		...
		<div class="erp-footer bg-dark text-white py-2 px-4 shadow-lg sticky-bottom flex-shrink-0">
			<div class="row align-items-center w-100">
				<div class="col-md-3 small">조회 업체: <span class="fw-bold text-info">{{ rowCount }}</span> 건</div>
...
const fetchData = async () => {
	...
	try {
		const res = await api.post('/api/hsst/HSST_610S_STR', {
			...searchForm,
...
const handleExcel = () => mainGrid?.download("xlsx", "거래처별매출추이.xlsx")
