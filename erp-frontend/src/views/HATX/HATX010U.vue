...
const fetchOptions = async () => {
	try {
		const [resU, resT, resP] = await Promise.all([
			...
			api.post('/api/ha00/HA00_00P_STR', { gubun: 'E0', gbncd: '120' }), // 매입 유형
			api.post('/api/ha00/HA00_00P_STR', { gubun: 'E0', gbncd: '190' })  // 지불 수단
		])
...
const save = async () => {
	...
	try {
		if (formData.slipyn === 'Y') {
			...
			if (inAmt !== Math.abs(Number(formData.amtsum.replace(/,/g, '')))) return vAlertError('매입액과 지불액이 일치하지 않습니다.')
		}
		await api.post('/api/hatx/HATX_010U_STR', {
			...formData, actkind: formData.taxno ? 'U1' : 'I1', cmpycd: authStore.cmpycd, taxkind: '100',
...
const handleTaxDelete = async () => {
	...
	try {
		await api.post('/api/hatx/HATX_010U_STR', { actkind: 'D1', cmpycd: authStore.cmpycd, taxkind: '100', taxym: formData.taxym, taxno: formData.taxno })
...
const loadDetail = async (row: any) => {
	try {
		...
		if (res.data?.[0]) {
			...
			const resItems = await api.post('/api/hatx/HATX_011U_STR', { actkind: 'S1', cmpycd: row.cmpycd, taxkind: '100', taxym: row.taxym, taxno: row.taxno })
...
function openHelp(type: string, idx?: number) {
	if (type === 'CUST') {
		Object.assign(modalProps, {
			title: '거래처 선택', path: '/api/ha00/HA00_00P_STR', data: { gubun: 'C4', cmpycd: authStore.cmpycd, code: formData.custnm },
		...
		Object.assign(modalProps, {
			title: '계정과목 선택', path: '/api/ha00/HA00_00P_STR', data: { gubun: 'A0', cmpycd: authStore.cmpycd, code: '13' },
		...
		Object.assign(modalProps, {
			title: '관리번호 선택', path: '/api/ha00/HA00_00P_STR', data: { gubun: 'E0', cmpycd: authStore.cmpycd, gbncd: '190' },
