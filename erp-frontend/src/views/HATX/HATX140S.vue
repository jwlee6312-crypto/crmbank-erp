	...
	try {
		...
		if (res.data?.length) {
			...
			const mainData = [
				...
				{ gubun: '봉 사 료', tot: d.col9, card: d.col10, cash: d.col11 }
			];
			mainGrid?.setData(mainData);

			const subData = [
				{ item: '세금계산서 교부 금액', amt: d.col12 },
				{ item: '계산서 교부 금액', amt: d.col13 }
			];
...
const print = () => {
	const params = new URLSearchParams({
		taxunit: searchForm.taxunit, yy: searchForm.yy, fmm: searchForm.fmm, tmm: searchForm.tmm, PRTGU: '1'
	}).toString();
