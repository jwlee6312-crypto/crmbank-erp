<!--	=============================================================
	...
	?ëÏÑ±?ºÏûê	: 2025.02.24
	?ëÏÑ±??    : AI Assistant
	?§Î™Ö        : ?¨ÏóÖ?•Î≥Ñ Îß§ÏûÖ ?∏Í∏àÍ≥ÑÏÇ∞???ÅÏÑ∏ ?¥Ïó≠ Ï°∞Ìöå
	=============================================================
-->
	...
	<div class="erp-container">
		<div class="erp-header d-flex justify-content-between align-items-center border-bottom bg-white py-2 px-3 sticky-top shadow-sm flex-shrink-0">
			<div class="fw-bold text-dark d-flex align-items-center" style="font-size: 14px;">
				...
				<span class="text-primary fw-bolder">Îß§ÏûÖ??(HATX030S)</span>
			</div>
				...
				<div class="card-body p-2 bg-light">
					<div class="d-flex flex-column gap-2 small">
						<div class="d-flex align-items-center flex-wrap gap-3">
							...
							<div class="d-flex align-items-center">
								<span class="erp-label"><i class="bi bi-dot"></i>?†Ìòï</span>
						...
						<div class="d-flex align-items-center flex-wrap gap-3">
							<div class="d-flex align-items-center">
								<span class="erp-label"><i class="bi bi-dot"></i>Í±∞ÎûòÏ≤?/span>
								<div class="d-flex align-items-center gap-1">
									<div class="input-group input-group-sm" style="width: 200px;">
										...
										<input v-model="searchForm.Fcustnm" type="text" class="form-control" @keydown.enter="openHelp('CUST', 'F')" placeholder="?úÏûë Í±∞ÎûòÏ≤? />
									...
									<div class="input-group input-group-sm" style="width: 200px;">
										...
										<input v-model="searchForm.Tcustnm" type="text" class="form-control" @keydown.enter="openHelp('CUST', 'T')" placeholder="Ï¢ÖÎ£å Í±∞ÎûòÏ≤? />
										<button class="btn btn-outline-secondary px-2" @click="openHelp('CUST', 'T')"><i class="bi bi-search"></i></button>
									</div>
								</div>
							</div>
							<div class="d-flex align-items-center">
								<span class="erp-label"><i class="bi bi-dot"></i>Ï¢ÖÎ•ò</span>
								<select v-model="searchForm.elcyn" class="form-select form-select-sm" style="width: 130px;">
									<option value="0">?ÑÏ≤¥</option>
									<option value="N">?ºÎ∞ò?∏Í∏àÍ≥ÑÏÇ∞??/option>
									<option value="Y">?ÑÏûê?∏Í∏àÍ≥ÑÏÇ∞??/option>
								</select>
							</div>
						</div>
					</div>
...
const search = async () => {
	if (!searchForm.SAUPCD) return vAlertError('?¨ÏóÖ?•ÏùÑ ?†ÌÉù?òÏÑ∏??')
	if (searchForm.gubun1 > searchForm.gubun2) return vAlertError('?†Ìòï Î≤îÏúÑÎ•??ïÏù∏?òÏÑ∏??')
...
const excel = () => {
	mainGrid?.download("xlsx", `Îß§ÏûÖ??${searchForm.stdymd}_${searchForm.endymd}.xlsx`)
}
...
function openHelp(type: string, target?: string) {
	if (type === 'CUST') {
		Object.assign(modalProps, {
			title: 'Í±∞ÎûòÏ≤??†ÌÉù', path: '/api/ha00/HA00_010S_STR',
			data: { gubun: 'C0', search_gubun: '010', cmpycd: authStore.cmpycd, search: '' },
			columns: [{ title: 'ÏΩîÎìú', field: 'custcd', width: 80 }, { title: 'Í±∞ÎûòÏ≤òÎ™Ö', field: 'custnm', width: 180 }],
			onConfirm: (d: any) => {
...
onMounted(() => {
	...
	if (mainGridRef.value) {
		mainGrid = new Tabulator(mainGridRef.value, {
			...
			columnDefaults: {
				headerSort: false,
				hozAlign: 'right', // ?? Í∏∞Î≥∏Í∞??∞Ï∏° ?ïÎ†¨
				vertAlign: "middle"
			},
			columns: [
				{
					title: "Î∞úÌñâ??, field: "pubymd", width: 90, hozAlign: "center", frozen: true,
					formatter: (cell) => { const v = cell.getValue(); return v ? `${v.slice(0,4)}-${v.slice(4,6)}-${v.slice(6,8)}` : '' }
				},
				{ title: "?¨ÏóÖ?êÎ≤à??, field: "custno", width: 110, hozAlign: "center", frozen: true },
				{ title: "?ÅÌò∏", field: "custnm", width: 150, frozen: true },
				{ title: "?†Ìòï", field: "typenm", width: 80, hozAlign: "center" },
				{ title: "?ÑÏûê", field: "elcyn", width: 50, hozAlign: "center" },
				{ title: "?ÅÏöî", field: "descnm", widthGrow: 1.5 },
				{
					title: "Í≥µÍ∏âÍ∞Ä", field: "supyamt", width: 100, hozAlign: "right", formatter: "money", formatterParams: { precision: 0 },
					bottomCalc: "sum", bottomCalcFormatter: "money", bottomCalcFormatterParams: { precision: 0 }
				},
				{
					title: "Î∂ÄÍ∞Ä??, field: "vatamt", width: 90, hozAlign: "right", formatter: "money", formatterParams: { precision: 0 },
					bottomCalc: "sum", bottomCalcFormatter: "money", bottomCalcFormatterParams: { precision: 0 }
				},
				{
					title: "?©Í≥Ñ", field: "totamt", width: 110, hozAlign: "right", formatter: "money", formatterParams: { precision: 0 },
					bottomCalc: "sum", bottomCalcFormatter: "money", bottomCalcFormatterParams: { precision: 0 },
					cssClass: "fw-bold"
				},
				{
					title: "Î≤àÌò∏", field: "TAX_REF", width: 110, hozAlign: "center",
				...
				{
					title: "?ÑÌëúÎ≤àÌò∏", field: "SLIP_REF", width: 150, hozAlign: "center",
					...
					cellClick: (e, cell) => {
						...
						tabStore.addTab({
							name: d.slipgu === '010' ? '?åÍ≥Ñ?ÑÌëúÍ¥ÄÎ¶? : '?ºÎ∞ò?ÑÌëúÍ¥ÄÎ¶?,
							path: path,
