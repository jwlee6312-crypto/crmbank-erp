<!--	=============================================================
	...
	?�성?�자	: 2025.02.24
	?�성??    : AI Assistant
	?�명        : 면세 계산?�의 매입/매출 ?�계 �?거래처별 집계 ?�황 조회 (HSOD100U ?��? UI ?�용)
	=============================================================
-->
	...
	<div class="erp-container">
		<div class="erp-header d-flex justify-content-between align-items-center border-bottom bg-white py-2 px-3 sticky-top shadow-sm flex-shrink-0">
			<div class="fw-bold text-dark d-flex align-items-center" style="font-size: 14px;">
				...
				<span class="text-primary fw-bolder">계산?�합계표 (HATX120S)</span>
			</div>
		...
		<div class="flex-grow-1 overflow-auto p-2 d-flex flex-column gap-2 bg-light">
			...
			<div class="card border shadow-sm flex-shrink-0 overflow-hidden d-flex flex-column bg-white">
				<div class="card-header bg-white py-1 px-3 border-bottom fw-bold small text-dark">
					<i class="bi bi-grid-3x3-gap-fill me-1 text-primary"></i> 계산??총괄 ?�계
				</div>
			...
			<div class="card border shadow-sm flex-grow-1 overflow-hidden d-flex flex-column bg-white">
				<div class="card-header bg-white py-1 px-3 border-bottom fw-bold small text-dark">
					<i class="bi bi-list-ul me-1 text-primary"></i> 거래처별 집계 ?�세
				</div>
...
const titleGbn = computed(() => searchForm.GUBUN === '100' ? '?? : '�?)
...
const search = async () => {
	try {
		...
		// 1. ?�계 ?�보 조회 (iogbn: 41 for Purchase Sum, 31 for Sales Sum)
		...
		if (searchForm.GUBUN === '200') {
			sumData = [
				{ category: '?�계', count: rawSum.col0, sheets: rawSum.col1, amount: rawSum.col2 },
				{ category: '?�업?�등록번??발행�?, count: rawSum.col3, sheets: rawSum.col4, amount: rawSum.col5 },
				{ category: '주�??�록번호 발행�?, count: rawSum.col4, sheets: rawSum.col5, amount: rawSum.col6 } // Following ASP logic overlap
			]
		} else {
			sumData = [
				{ category: '?�업?�등록번??발행�?, count: rawSum.col0, sheets: rawSum.col1, amount: rawSum.col2 }
			]
		}
		summaryGrid?.setData(sumData)

		// 2. ?�세 목록 조회 (iogbn: 42 for Purchase List, 32 for Sales List)
	...
	if (summaryGridRef.value) {
		summaryGrid = new Tabulator(summaryGridRef.value, {
			...
			columns: [
				...
				{ title: `�?{titleGbn.value}처수`, field: "count", width: 150, hozAlign: "right", formatter: "money", formatterParams: { precision: 0 } },
				{ title: "매수", field: "sheets", width: 150, hozAlign: "right", formatter: "money", formatterParams: { precision: 0 } },
				{ title: `�?{titleGbn.value}금액`, field: "amount", width: 200, hozAlign: "right", formatter: "money", formatterParams: { precision: 0 } }
			]
		})
	}
	...
	if (mainGridRef.value) {
		mainGrid = new Tabulator(mainGridRef.value, {
			...
			columns: [
				...
				{ title: "?�업?�번??, field: "custno", width: 110, hozAlign: "center" },
				{ title: "?�호", field: "custnm", width: 150, cssClass: "fw-bold text-primary" },
				{ title: "?�태", field: "custkind", width: 100 },
				...
				{ title: `�?{titleGbn.value}금액`, field: "amt", width: 130, hozAlign: "right", formatter: "money", formatterParams: { precision: 0 } }
			]
		})
	}
})
