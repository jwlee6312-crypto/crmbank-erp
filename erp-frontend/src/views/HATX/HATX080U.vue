	...
	<div class="erp-container">
		<div class="erp-header d-flex justify-content-between align-items-center border-bottom bg-white py-2 px-3 sticky-top shadow-sm flex-shrink-0">
			<div class="fw-bold text-dark d-flex align-items-center" style="font-size: 14px;">
				...
				<span class="text-primary fw-bolder">전자세금계산서발송 (HATX080U)</span>
			</div>
			<div class="btn-group-erp d-flex gap-1">
				<button class="btn-erp btn-save" @click="handleBatchAction('U0')">
					<i class="bi bi-send"></i> {{ searchForm.cfmyn === 'N' ? '전송처리' : '재전송처리' }}
				</button>
			...
			<div class="card border shadow-sm bg-white overflow-hidden">
				<div class="card-body p-2 bg-light">
					<div class="d-flex align-items-center flex-wrap gap-3 small">
						...
						<div class="d-flex align-items-center">
							<span class="erp-label"><i class="bi bi-dot"></i>조회대상</span>
...
const handleBatchAction = async (gbn: string) => {
	...
	let msg = ''
	if (gbn === 'U0') {
		msg = searchForm.cfmyn === 'N' ? '선택한 세금계산서를 전송 하시겠습니까?' : '선택한 세금계산서를 재전송 하시겠습니까?'
	} else {
	...
	try {
		for (const row of selectedRows) {
			...
			if (res.data && res.data[0] && res.data[0].col0 === 'N') {
				vAlertError(res.data[0].col2 || '처리 중 오류가 발생했습니다.')
				break
			}
		}
