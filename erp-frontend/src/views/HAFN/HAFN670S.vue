<!--
	=============================================================
	?ÑÎ°úÍ∑∏Îû®Î™?: ÎØ∏Ï?Í∏âÍ∏à?ÑÌô© (HAFN670S)
	?ëÏÑ±?ºÏûê	: 2025.02.24
	?§Î™Ö        : Í±∞ÎûòÏ≤òÎ≥Ñ ÎØ∏Ï?Í∏âÍ∏à Î∞úÏÉù, ÏßÄÎ∂? ?îÏï° ?ÑÌô© Ï°∞Ìöå (HSOD100U ?úÏ? ?îÏûê??Î∞??åÎ¨∏???êÏπô ?ÅÏö©)
	=============================================================
-->
  ...
  <div class="erp-container d-flex flex-column h-100 bg-white">
    ...
    <div class="erp-header d-flex justify-content-between align-items-center flex-shrink-0 border-bottom bg-white py-2 px-3 sticky-top shadow-sm">
      <div class="fw-bold ps-1 text-dark d-flex align-items-center" style="font-size: 14px;">
        ...
        <span class="text-primary fw-bolder">ÎØ∏Ï?Í∏âÍ∏à?ÑÌô© (HAFN670S)</span>
      </div>
    ...
    <div class="flex-grow-1 overflow-hidden p-2 d-flex flex-column gap-2 bg-light main-content-wrapper">
          ...
          <table class="erp-table-dense" width="100%">
            ...
            <tbody>
              <tr>
                ...
                <th class="text-center bg-light border-end">Í±???Ï≤?/th>
                <td>
                  <div class="d-flex align-items-center gap-1 px-1">
                    <div class="input-group input-group-sm" style="width: 220px;">
                      ...
                      <input v-model="search_form.custnmfr" type="text" class="form-control" placeholder="?úÏûë Í±∞ÎûòÏ≤? @keydown.enter="open_help('CUSTFR')" />
                    ...
                    <div class="input-group input-group-sm" style="width: 220px;">
                      ...
                      <input v-model="search_form.custnmto" type="text" class="form-control" placeholder="Ï¢ÖÎ£å Í±∞ÎûòÏ≤? @keydown.enter="open_help('CUSTTO')" />
      ...
      <div class="card border shadow-sm flex-grow-1 d-flex flex-column overflow-hidden grid-container-right">
        <div class="card-header bg-white py-1 px-3 border-bottom text-secondary">
          <span class="fw-bold small"><i class="bi bi-grid-3x3-gap-fill me-2 text-primary"></i> Í±∞ÎûòÏ≤òÎ≥Ñ ÎØ∏Ï?Í∏âÍ∏à ?¥Ïó≠</span>
        </div>
...
const excel = () => maingrid?.download("xlsx", `ÎØ∏Ï?Í∏âÍ∏à?ÑÌô©_${today}.xlsx`)
...
// ?ÅÏÑ∏ ?¥Îèô (HAFN650SÎ°??¥Îèô)
const go_detail = (row: any) => {
    const pgmid = 'HAFN650S'
    add_dynamic_route(pgmid, 'ÎØ∏Ï?Í∏âÍ∏à?ÅÏÑ∏?ÑÌô©', 'HAFN')
	router.push({
		path: `/${pgmid}`,
...
function open_help(type: string) {
	...
	Object.assign(modal_props, {
		title: 'Í±∞ÎûòÏ≤??†ÌÉù', path: '/api/ha00/HA00_00P_STR',
...
_on_mounted(() => {
	if (main_grid_ref.value) {
		maingrid = new tabulator(main_grid_ref.value, {
			...
			columns: [
				{
					title: "Í±∞ÎûòÏ≤?, field: "custcd", width: 100, hozAlign: "center",
				...
				{
					title: "Í±∞ÎûòÏ≤òÎ™Ö", field: "custnm", width: 300,
					formatter: (cell) => `<span class="text-primary text-decoration-underline cursor-pointer">${cell.getValue()}</span>`,
					cellClick: (e, cell) => go_detail(cell.getData())
				},
				{ title: "?ÖÍ∏à?Ä??Í≥ÑÏ¢å", field: "bank_info", minWidth: 300 },
				{
					title: "Î∞úÏÉù??, field: "upyamt", width: 150, hozAlign: "right",
					formatter: "money", formatterParams: { precision: 0 },
					bottomCalc: "sum", bottomCalcFormatter: "money", bottomCalcFormatterParams: { precision: 0 }
				},
				{
					title: "ÏßÄÎ∂àÏï°", field: "payamt", width: 150, hozAlign: "right",
					formatter: "money", formatterParams: { precision: 0 },
					bottomCalc: "sum", bottomCalcFormatter: "money", bottomCalcFormatterParams: { precision: 0 }
				},
				{
					title: "????, field: "janamt", width: 150, hozAlign: "right",
					formatter: "money", formatterParams: { precision: 0 },
					bottomCalc: "sum", bottomCalcFormatter: "money", bottomCalcFormatterParams: { precision: 0 },
