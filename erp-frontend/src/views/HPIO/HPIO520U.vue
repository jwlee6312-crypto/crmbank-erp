...
async function fetchDetail(row: any) {
  ...
  try {
    const res = await api.post('/api/hpio/HPIO_521U_STR', { actkind: 'S', cmpycd: authStore.cmpycd, ioym: row.ioym, iono: row.iono, linecd: row.linecd });
...
async function saveData() {
  ...
  try {
    ...
    const resM = await api.post('/api/hpio/HPIO_520U_STR', { ...masterData, actkind, cmpycd: authStore.cmpycd, userid: authStore.userid });
...
const onLineChange = async () => {
  ...
  try {
    const res = await api.get('/api/hp00/HP00_000S_STR', { params: { gubun: 'G0', cmpycd: authStore.cmpycd, gbncd: masterData.linecd } });
