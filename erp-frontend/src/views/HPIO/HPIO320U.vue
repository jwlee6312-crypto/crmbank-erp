...
async function fetchDetail(row: any) {
  ...
  if (row.linecd) {
    api.post('/api/hp00/HP00_000S_STR', { gubun: 'G0', cmpycd: authStore.cmpycd, gbncd: row.linecd }).then(res => {
      processData.value = normalizeData(res.data).map((i: any) => ({ progcd: i.progcd, prognm: i.prognm }));
    });
  }
  try {
...
onMounted(() => {
  ...
  api.post('/api/hp00/HP00_000S_STR', { gubun: 'L0', cmpycd: authStore.cmpycd, gbncd: 'Y' }).then(res => {
    lineData.value = normalizeData(res.data).map((i: any) => ({ linecd: i.linecd, linenm: i.linenm }));
  });
  // 창고 데이터 로드
  api.post('/api/hs00/HS00_000S_STR', { gubun: 'W0', cmpycd: authStore.cmpycd }).then(res => {
    warehouseData.value = normalizeData(res.data).map((i: any) => ({ whcd: i.whcd, whnm: i.whnm }));
  });
