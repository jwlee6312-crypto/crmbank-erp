...
const handleProcess = async () => {
    ...
    if (closingInfo.WCLSYM >= YM) {
        return vAlertError(`원가마감(${closingInfo.WCLSYM}) 되었습니다. 원가마감 취소 후 작업 바랍니다.`)
    }

    // 2. 확인 메시지
    const confirmMsg = closeData.WKGBN === 'Y'
        ? `${closeData.YY}년 ${closeData.MM}월 마감 작업을 하시겠습니까?`
        : `${closeData.YY}년 ${closeData.MM}월 마감 취소 작업을 하시겠습니까?`

    if (!confirm(confirmMsg)) return

    try {
        const actkind = closeData.WKGBN === 'Y' ? 'A0' : 'D0'
        const res = await api.post('/api/hpcl/HPCL_100U_STR', {
            ACTKIND: actkind,
        ...
        if (res.data?.[0]?.RTN_CD === '000' || res.data?.[0]?.CODE === '000') {
            vAlert('정상적으로 작업이 완료되었습니다.')
            fetchClosingStatus()
        } else {
            vAlertError(res.data?.[0]?.RTN_MSG || res.data?.[0]?.MESSAGE || '처리 중 오류 발생')
        }
    } catch (e) {
...
.btn-save:hover { background-color: #374fc7 !important; transform: translateY(-1px); box-shadow: 0 4px 12px rgba(67, 97, 238, 0.3); }
...
.form-select { height: 38px; font-size: 14px; }
</style>
