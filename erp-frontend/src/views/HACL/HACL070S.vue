<!--
	=============================================================
	?ÑÎ°úÍ∑∏Îû®Î™?: ?êÏùµÍ≥ÑÏÇ∞???ÑÎ°ú?ùÌä∏) (HACL070S)
	?ëÏÑ±?ºÏûê	: 2025.02.24
	?§Î™Ö        : HSOD100U ?úÏ? Í∑∏Î¶¨???®ÌÑ¥??Ï§Ä?òÌïò???ÑÎ°ú?ùÌä∏Î≥??êÏùµÍ≥ÑÏÇ∞??Ï°∞Ìöå
	=============================================================
-->
  ...
  <div class="erp-container d-flex flex-column h-100 bg-white">
    ...
    <div class="erp-header d-flex justify-content-between align-items-center flex-shrink-0 border-bottom">
      <div class="fw-bold ps-1 text-dark d-flex align-items-center" style="font-size: 14px;">
        ...
        <span class="text-primary fw-bolder">?êÏùµÍ≥ÑÏÇ∞???ÑÎ°ú?ùÌä∏) (HACL070S)</span>
      </div>
        ...
        <div class="card-body p-2 bg-white">
          <div class="d-flex align-items-center gap-3 small">
            <div class="d-flex align-items-center">
              <span class="erp-label" style="min-width: 70px;"><i class="bi bi-dot text-primary"></i>?ÑÎ°ú?ùÌä∏</span>
              <div class="input-group input-group-sm shadow-sm" style="width: 280px;">
                ...
                <input v-model="searchForm.prjnm" type="text" class="form-control" @keydown.enter="openHelp('PRJ')" placeholder="?ÑÎ°ú?ùÌä∏ ?†ÌÉù" />
            ...
            <div class="d-flex align-items-center">
              ...
              <div class="d-flex align-items-center gap-1">
                ...
                <span class="ms-1 fw-bold">??ÍπåÏ?</span>
              </div>
            </div>
          </div>
        </div>
...
const initGrid = () => {
  ...
  grid = new Tabulator(tableRef.value, {
    ...
    columns: [
      {
        title: "Í≥? Î™?, field: "formatted_nm", widthGrow: 2,
        formatter: "html",
      ...
      },
      {
        title: "?ÑÏõî?ÑÍ≥Ñ",
        columns: [
          { title: "Í∏? ??, field: "bamt", width: 150, hozAlign: "right", formatter: "money", formatterParams: { precision: 0 } },
      ...
      },
      {
        title: "?? ??,
        columns: [
          { title: "Í∏? ??, field: "camt", width: 150, hozAlign: "right", formatter: "money", formatterParams: { precision: 0 }, cssClass: "bg-light-subtle fw-bold" },
      ...
      },
      {
        title: "?? Í≥?,
        columns: [
          { title: "Í∏? ??, field: "tamt", width: 150, hozAlign: "right", formatter: "money", formatterParams: { precision: 0 } },
...
const search = async () => {
  if (!searchForm.prjcd) return vAlertError('?ÑÎ°ú?ùÌä∏Î•??†ÌÉù?òÏÑ∏??');
...
const excel = () => grid?.download("xlsx", `?êÏùµÍ≥ÑÏÇ∞???ÑÎ°ú?ùÌä∏_${searchForm.yy}${searchForm.mm}.xlsx`)
...
function openHelp(type: string) {
  Object.assign(modalProps, {
    title: '?ÑÎ°ú?ùÌä∏ ?†ÌÉù', path: '/api/ha00/HA00_03P_STR', defaultField: 'col1',
    data: { custgbn: '060', cmpycd: authStore.cmpycd, search: searchForm.prjnm },
    columns: [{ title: 'ÏΩîÎìú', field: 'col0', width: 80 }, { title: '?ÑÎ°ú?ùÌä∏Î™?, field: 'col1', width: 250 }],
    onConfirm: (d: any) => {
