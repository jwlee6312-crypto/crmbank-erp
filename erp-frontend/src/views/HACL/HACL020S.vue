...
const search = async () => {
	try {
		...
		const processedData = rawData.map((row: any, idx: number) => {
			...
			const assettot = Number(row.col7 || row.assettot || 0)
			const assettotb = Number(row.col8 || row.assettot_b || 0)
			...
			if (acctCd.substring(1, 7) === "000000" || ["1990000", "2980000", "3980000", "3990000"].includes(acctCd)) {
				formattedNm = `[ ${acctNm} ]`
				levelClass = "fw-bolder text-dark bg-light-subtle"
			} else if (acctCd.substring(2, 7) === "00000") {
				formattedNm = `${sNUM[i] || i}. ${acctNm}`
				i++
				if (assettot !== 0) rate = (camtr / assettot) * 100
				if (assettotb !== 0) rateB = (bamtr / assettotb) * 100
				levelClass = "fw-bold"
			...
			// 다음 데이터 확인하여 인덱스 리셋 (ASP의 Rs.MoveNext 이후 리셋 로직)
...
const excel = () => {
	...
	mainGrid?.download("xlsx", `대차대조표_${searchForm.yy}${searchForm.mm}_${today}.xlsx`)
}

const print = () => {
	const params = `yy=${searchForm.yy}.mm=${searchForm.mm}&PRTGU=1`
	window.open(`/api/hacl/HACL_020P?${params}`, 'BalanceSheetPrint', 'width=800,height=800,scrollbars=yes')
}
...
:deep(.ps-5) { padding-left: 2rem !important; }
</style>
