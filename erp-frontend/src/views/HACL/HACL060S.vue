<!--
	...
	작성일자	: 2025.02.24
	작성자	    : AI Assistant
	설명        : 지정된 년월의 제조원가명세서 (전월누계, 당월, 누계) 조회 및 분석
	=============================================================
-->
...
const search = async () => {
	try {
		...
		const processedData = rawData.map((row: any, idx: number) => {
			...
			const bamt = Number(row.col2 || row.bamt || 0)
			const camt = Number(row.col3 || row.camt || 0)
			const tamt = Number(row.col4 || row.tamt || 0)

			const rstyn = row.col5 || row.rstyn
			const mfTotB = Number(row.col6 || row.mftot_b || 0)
			const mfTotC = Number(row.col7 || row.mftot_C || 0)
			const mfTotT = Number(row.col8 || row.mftot_T || 0)
			...
			if (acctCd === "4113000" || acctCd === "4160000") {
				...
				i++
				if (mfTotB !== 0) brate = (bamt / mfTotB) * 100
				if (mfTotC !== 0) crate = (camt / mfTotC) * 100
				if (mfTotT !== 0) trate = (tamt / mfTotT) * 100
				levelClass = "fw-bold"
			...
			if (nextRow) {
				...
				if (acctCd.substring(0, 3) !== nextCd.substring(0, 3)) k = 1
...
const excel = () => {
	...
	mainGrid?.download("xlsx", `제조원가명세서_당월_${searchForm.yy}${searchForm.mm}_${today}.xlsx`)
}

const print = () => {
	const params = `yy=${searchForm.yy}.mm=${searchForm.mm}&PRTGU=1`
	window.open(`/api/hacl/HACL_060P?${params}`, 'MonthlyMfgCostPrint', 'width=800,height=800,scrollbars=yes')
}
