<!--
	=============================================================
	프로그램명	: 외부매입전표취소 (External Purchase Slip Cancellation)
	작성일자	: 25.02.24
	작성자	    : AI Assistant
	설명        : ASP(HSIO_141U) 소스 기반 재작성 (단일 그리드 표준 + 팝업 표준화)
	=============================================================
-->
    ...
    <div class="flex-grow-1 overflow-hidden p-2">
      <div class="card border shadow-sm h-100 d-flex flex-column bg-white">
        <div class="card-header bg-white py-1 px-3 border-bottom d-flex align-items-center justify-content-between">
          <span class="fw-bold small text-dark"><i class="bi bi-grid-3x3-gap-fill me-2 text-primary"></i> 취소 대상 외부 매입 전표 내역</span>
...
// 마감 정보
const clsInfo = reactive({ SCLSYM: '', PCLSYM: '' })
...
async function fetchClosingInfo() {
  try {
    ...
    if (res.data?.[0]) {
      ...
      clsInfo.PCLSYM = res.data[0].PCLSYM || ''
    }
  } catch (e) {}
}
...
async function handleCancel() {
  ...
  for (const item of items) {
    ...
    const slipYmd = String(item.JSANYMD || '');

    if (clsInfo.SCLSYM && slipYm <= clsInfo.SCLSYM) {
        return vAlertError(`영업정보가 마감되었습니다. 취소할 수 없습니다. [전표:${item.SLIPYMD}-${item.SLIPNO}]`);
    }
    if (clsInfo.PCLSYM && slipYmd <= clsInfo.PCLSYM) {
        return vAlertError(`생산정보가 마감이 되었습니다. 해당 전표일자로 작업할 수 없습니다. [전표:${item.SLIPYMD}-${item.SLIPNO}]`);
    }
  }
      ...
      columns: [
        ...
        { title: '전표번호', field: 'SLIPNO_FULL', width: 150, hozAlign: 'center', cssClass: 'fw-bold text-primary cursor-pointer',
          ...
          cellClick: (e, cell) => {
            ...
            window.open(`../HASL/HASL_SLIP_PRINT_OUT.asp?SLIPGU=010&SLIPYMD=${d.SLIPYMD}&SLIPNO=${d.SLIPNO}&DEPTCD=${d.UDEPTCD}`, '전표인쇄', 'left=10,top=10,width=700,height=650,scrollbars=yes');
          }
        },
        ...
        { title: '합계', field: 'JSANSUM', width: 120, hozAlign: 'right', formatter: 'money', formatterParams: { precision: 0 }, cssClass: 'bg-light fw-bold', mutatorData: (v, d) => Number(d.SPYAMT || 0) + Number(d.VATAMT || 0) }
      ]
    })
  }
  nextTick(() => fetchList())
...
<style scoped>
.hsio141u-wrapper { height: 100%; overflow: hidden; font-family: 'Pretendard', sans-serif; }
