	...
	<div class="hsio140u-wrapper d-flex flex-column h-100 bg-white p-0">
		...
		<div class="erp-header d-flex justify-content-between align-items-center border-bottom bg-white py-2 px-3 sticky-top shadow-sm">
			...
			<div class="btn-group-erp d-flex gap-1">
				...
				<button class="btn-erp btn-save" @click="handleSave">전표취소</button>
			</div>
		</div>
				...
				<div class="card-body py-2 px-3">
					<div class="d-flex align-items-center gap-4">
						<div class="d-flex align-items-center gap-2">
							<span class="fw-bold small text-dark">입고부서</span>
						...
						<div class="d-flex align-items-center gap-2">
							<span class="fw-bold small text-dark">발행일자</span>
...
async function loadClsInfo() {
  try {
    ...
    if (res.data?.length) {
        const d = res.data[0];
        clsInfo.CLSYMD = d.CLSYMD; clsInfo.SCLSYM = d.SCLSYM;
    }
  } catch (e) {}
}
...
async function fetchDetail(cust: any) {
  ...
  } catch (e) { vAlertError('전표 내역 조회 실패') }
}
...
async function handleSave() {
  ...
  try {
    for (const item of items) {
        await api.post('/api/hsio/HSIO_140U_STR', {
      ...
      columns: [
        { title: '선택', field: 'PROCYN', hozAlign: 'center', width: 80, formatter: 'tickCross', editor: true,
          cellClick: (e, cell) => {
              ...
              if (d.JSANYMD <= clsInfo.CLSYMD) { vAlertError('회계 마감된 자료입니다.'); cell.setValue('N'); return; }
          }
        },
        ...
        { title: '발행부서', field: 'DEPTNM', minwidth: 130 },
...
const modalVisible = ref(false); const modalProps = reactive<any>({ title: '', path: '', onConfirm: () => {} })
 ...
 <style scoped>
.hsio140u-wrapper { height: 100%; overflow: hidden; }
