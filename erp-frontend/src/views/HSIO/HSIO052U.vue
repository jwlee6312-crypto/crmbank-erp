<!--
	...
	프로그램명	: 발주등록 (HSIO052U)
	작성일자	: 2025.02.24
	설명        : 구매 발주 마스터/상세 관리 (HSOD100U 표준 구조 및 소문자 원칙 적용)
	=============================================================
-->
  ...
  <div class="erp-container d-flex flex-column h-100 bg-white">
    ...
    <div class="erp-header d-flex justify-content-between align-items-center flex-shrink-0 border-bottom">
      <div class="fw-bold ps-1 text-dark d-flex align-items-center" style="font-size: 14px;">
        ...
        <span class="text-primary fw-bolder">발주등록 (HSIO052U)</span>
      </div>
          ...
          <table class="erp-table-dense" width="100%">
            ...
            <tbody>
              <tr>
                ...
                <th class="text-center bg-light">발주거래처</th>
                <td>
          ...
          <div class="card border shadow-sm flex-shrink-0 overflow-hidden">
            <div class="card-body p-0 bg-white">
              <table class="erp-table-dense w-100">
                ...
                <tbody>
                  <tr>
                    <th class="bg-light text-center">발주번호</th>
                    <td><input v-model="form_02.balno" class="form-control bg-light text-primary fw-bold text-center" readonly placeholder="자동생성" /></td>
                  ...
                  <tr>
                    ...
                    <td colspan="3"><input v-model="form_02.email" class="form-control" /></td>
                    <th class="required bg-light text-center">담당자</th>
                    <td>
                      <select v-model="form_02.bal_userid" class="form-select">
...
const displayIoNo = computed(() => (!form_02.balno || form_02.balno === '0000') ? '' : form_02.balno)
...
async function search() {
  try {
    const res = await api.post('/api/hsio/HSIO_052U_STR', { actkind: 's1', fromdt: form_01.fromdt.replace(/-/g, ''), todt: form_01.todt.replace(/-/g, ''), custnm: form_01.custnm, balgb: form_02.balgb });
...
const handleOpenHelp = (type: string, target?: any) => {
  if (type === 'CUST') openHelp('CUST', (d) => { form_02.custcd = d.custcd; form_02.custnm = d.custnm; form_02.email = d.email || '' });
  else if (type === 'ITEM') openHelp('ITEM', (d) => target.update({ itemcd: d.itemcd, itemnm: d.itemnm, itsize: d.itsize, unitnm: d.unitnm, price: d.incost || 0, balqty: 1, balamt: d.incost || 0, _status: '입력', _state: 'NEW' }));
  else if (type === 'GRID_CUST') openHelp('CUST', (d) => target.update({ scustcd: d.custcd, scustnm: d.custnm, _status: target.getData()._state === 'EXIST' ? '수정' : '입력' }));
}
...
function initialize() {
  resetForm(form_02);
  Object.assign(form_02, { cmpycd: authStore.cmpycd, balno: '0000', balymd: today, reqymd: today, deptcd: authStore.deptcd, deptnm: authStore.deptnm, bal_userid: authStore.userid, flag: 'n', balgb: '2' });
...
async function handleFullDelete() {
  ...
  try {
    await api.post('/api/hsio/HSIO_052U_DELETE', { balno: form_02.balno });
