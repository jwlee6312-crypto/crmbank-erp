<template>
  ...
  <div class="hspl100u-wrapper d-flex flex-column h-100 bg-white p-0">
    <!-- 🚀 1. 상단 액션 바 -->
            ...
            <tbody>
              <tr>
                ...
                <td style="width: 220px;">
                  <div class="input-group input-group-sm">
                    <input v-model="searchData.USERID" type="text" class="form-control text-center bg-light" style="max-width: 60px;" readonly />
    ...
    <div class="erp-footer bg-dark text-white py-2 px-4 shadow-lg sticky-bottom">
      <div class="row align-items-center text-center">
        <div class="col-md-2 small border-end border-secondary">조회 품목: <span class="fw-bold text-info">{{ activeItemCount }}</span> 건</div>
        <div class="col-md-10 text-end">
          <span class="fs-5 fw-light">연간 계획 총 수량: <span class="fw-bold text-white ms-2">{{ formatNumber(grandTotal) }}</span></span>
        </div>
      </div>
    </div>
...
function openHelp(type: string) {
  // 🛠️ 경로 수정: /api/comm -> /api/ha00
  const commonPath = '/api/ha00/HA00_00P_STR'
  if (type === 'DEPT') {
    Object.assign(modalProps, {
      title: '영업부서 선택', path: commonPath, defaultField: 'DEPTNM',
    ...
    Object.assign(modalProps, {
      title: '영업사원 선택', path: commonPath, defaultField: 'USERNM',
...
<style scoped>
.hspl100u-wrapper { height: 100%; overflow: hidden; font-family: 'Pretendard', sans-serif; }
