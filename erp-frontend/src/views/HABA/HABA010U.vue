<!--
	...
	작성일자	: 2025.03.14
	설명        : 계정과목 등록 및 속성 설정 (완전 소문자 원칙 적용)
	=============================================================
-->
          ...
          <table class="erp-table-full border-0">
            <tbody>
              ...
              <tr>
                ...
                <th class="bg-light text-center">전표발행</th>
                <td>
...
function openhelp(type: string) {
  if (type === 'upacct') {
    commonopenhelp('ACCT', (d) => {
        ...
        masterform.upacct = n.acctcd; masterform.upacct_t = n.acctnm;
    });
  }
}
  ...
  if (maingridelement.value) {
    maingrid = new Tabulator(maingridelement.value, {
      ...
      columns: [
        ...
        { title: "계정과목명", field: "acctnm", minWidth: 200, hozAlign: "left" },
        { title: "차대", field: "typedc", width: 80, formatter: (c) => c.getValue()?.toLowerCase() === 'D' ? '차변' : '대변' },
        { title: "전표", field: "slipyn", width: 80, formatter: (c) => c.getValue()?.toLowerCase() === 'Y' ? '발행' : '미발행' },
        { title: "사용", field: "useyn", width: 80, formatter: (c) => c.getValue()?.toLowerCase() === 'Y' ? 'o' : 'x' }
      ]
    });
