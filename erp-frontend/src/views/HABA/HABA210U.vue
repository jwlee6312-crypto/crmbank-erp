<!--
	...
	작성일자	: 2025.02.24
	작성자	    : AI Assistant
	설명        : 계정별 기초 자료(이월/당월) 등록 및 관리 (표준 팝업 적용)
	=============================================================
-->
...
const save = async () => {
	if (!masterForm.yyyy || !masterForm.mm) return vAlert('회계년월을 선택해 주십시요.')
	if (!masterForm.acctcd) return vAlert('계정과목을 선택해 주십시요.')
	if (!masterForm.mmgbn) return vAlert('회계구분을 선택해 주십시요.')
...
/**
 * 💡 계정과목 도움창 호출 (useCommonHelp 표준 방식)
 */
	...
	if (mainGridRef.value) {
		mainGrid = new Tabulator(mainGridRef.value, {
			...
			columns: [
				...
				{ title: "계 정 과 목 명", field: "col3", minWidth: 200 },
				{ title: "차 변", field: "col5", width: 150, hozAlign: "right", formatter: "money", formatterParams: { precision: 0 } },
				{ title: "대 변", field: "col6", width: 150, hozAlign: "right", formatter: "money", formatterParams: { precision: 0 } },
				{ title: "잔 액", field: "col7", width: 150, hozAlign: "right", formatter: "money", formatterParams: { precision: 0 } }
			],
		})
